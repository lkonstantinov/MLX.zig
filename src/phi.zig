...
pub const Attention = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, parent: []const u8, name: []const u8, config: *const PhiConfig, weights_hash: *std.StringHashMap(*mlx.Array), stream: mlx.Stream) !Self {
        ...
        const qkv_proj = try mlx.Linear.init(allocator, key, "qkv_proj", false, config.quantization, weights_hash, stream);
...
pub const MLP = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, parent: []const u8, name: []const u8, intermediate: c_int, quant_config: ?mlx.QuantConfig, weights_hash: *std.StringHashMap(*mlx.Array), stream: mlx.Stream) !Self {
        var allocs_to_free = std.ArrayList([]const u8).init(allocator);
        const key = try allocJoin(allocator, parent, name);
        try allocs_to_free.append(key);
        const gate_up_proj = try mlx.Linear.init(allocator, key, "gate_up_proj", false, quant_config, weights_hash, stream);
...
pub const TransformerBlock = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, parent: []const u8, layer_idx: usize, config: *const PhiConfig, weights_hash: *std.StringHashMap(*mlx.Array), stream: mlx.Stream) !Self {
        var allocs_to_free = std.ArrayList([]const u8).init(allocator);
        ...
        const mlp = try MLP.init(allocator, key, "mlp", config.intermediate_size, config.quantization, weights_hash, stream);
...
pub const Phi4Model = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, parent: []const u8, config: *const PhiConfig, weights_hash: *std.StringHashMap(*mlx.Array), stream: mlx.Stream) !Self {
        var allocs_to_free = std.ArrayList([]const u8).init(allocator);
...
pub const Model = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config: *const PhiConfig, weights_hash: *std.StringHashMap(*mlx.Array), stream: mlx.Stream) !Self {
        const model = try Phi4Model.init(allocator, "model", config, weights_hash, stream);
